...
export const UserProfile = ({ name, role, image }: UserProfileProps) => {
  return (
    <div className="flex items-center gap-5">
      <img src={image} alt={name} className="w-11 h-11 rounded-[50%]" />
      <div className="flex flex-col max-sm:hidden">
        <span className="font-bold text-sm text-neutral-700">{name}</span>
        <span className="font-semibold text-xs text-[#565656]">{role}</span>
      </div>
      <svg
        width="20"
        height="20"
